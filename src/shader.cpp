...
void Program::update() const
{
  ...
  glUniformMatrix4fv(uniView, 1, GL_FALSE, glm::value_ptr(thomas::object::camera->getView()));

  GLint uniProj = glGetUniformLocation(handle(), "proj");
  glUniformMatrix4fv(uniProj, 1, GL_FALSE, glm::value_ptr(thomas::object::camera->getProjection(static_cast<float>(screenWidth), static_cast<float>(screenHeight))));

  GLint uniCam = glGetUniformLocation(handle(), "camera");
  if (uniCam >= 0)
    glUniform3fv(uniCam, 1, glm::value_ptr(thomas::object::camera->getPosition()));
